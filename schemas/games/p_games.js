...
const P_GamesSchema = new Schema({
...
P_GamesSchema.statics.getGamesBySeasonNumber = async function (seasonNumber) {
...
P_GamesSchema.statics.getSingleGame = async function (gameId) {
  const game = await this.findOne({ _id: gameId });
  return game;
};

// 4. query for a teams games by game type
P_GamesSchema.statics.getTeamsGames = async function (
  seasonNumber,
  gameType,
  teamAcronym
) {
...
// 5. query for segment of games for ticker or scoreboard
P_GamesSchema.statics.getSelectedGames = async function (
  seasonNumber,
  beginIndex,
  howManyGamesToGet
) {
...
P_GamesSchema.statics.getFieldData = async function (
  seasonNumber,
  parametersList,
  requestedDataObject
) {
...
P_GamesSchema.statics.getTeamsGameStats = async function (gameId, homeOrAway) {
...
P_GamesSchema.statics.getCsvGameData = async function (seasonNumber, gameReturnCount = 10) {
...
const P_Game = models.p_game || model("p_game", P_GamesSchema);

export default P_Game;
