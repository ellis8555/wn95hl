...
const dbCallFrom = "boxscores/[[...boxscores]]";
import GeneralLogo from "@/components/server/Logos/GeneralLogo";

async function page({params}) {
  const { boxscores } = params;
  const boxscoresLength = boxscores.length;

  switch(boxscoresLength){    
    // url includes league, season-number, and game ID           
    case 3:
      // extract and place into variables 
      const [leagueName, seasonNumber, gameId] = boxscores;
        ...
        if(gamesOtherData == null){
          throw new Error('There is an issue with the game ID number')
        }
      ...
      // return loading in any case where either league, season-number, or game ID are missing from the url
      default:
        return (
          <div className="text-center text-slate-300  w-11/12 md:w-3/4 mx-auto">
            <div>A leagues name, season-number, and game ID are required to view a particular game's boxscore</div>
          {/* team logos */}
          <div className="flex flex-row justify-center items-center gap-12 pt-3 md:pt-6 lg:w-1/2 mx-auto">
            <div className="w-5/12 flex justify-end ">
