...
const V_GamesSchema = new Schema({
...
V_GamesSchema.statics.getGamesBySeasonNumber = async function (seasonNumber) {
...
V_GamesSchema.statics.getSingleGame = async function (gameId) {
  const game = await this.findOne({ _id: gameId });
  return game;
};

// 4. query for a teams games by game type
V_GamesSchema.statics.getTeamsGames = async function (
  seasonNumber,
  gameType,
  teamAcronym
) {
...
// 5. query for segment of games for ticker or scoreboard
V_GamesSchema.statics.getSelectedGames = async function (
  seasonNumber,
  beginIndex,
  howManyGamesToGet
) {
...
V_GamesSchema.statics.getFieldData = async function (
  seasonNumber,
  parametersList,
  requestedDataObject
) {
...
V_GamesSchema.statics.getTeamsGameStats = async function (gameId, homeOrAway) {
...
V_GamesSchema.statics.getCsvGameData = async function (seasonNumber, gameReturnCount = 10) {
...
const V_Game = models.v_game || model("v_game", V_GamesSchema);

export default V_Game;
