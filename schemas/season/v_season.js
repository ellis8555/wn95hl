...
const V_LeagueSchema = new Schema({
  ...SeasonSchema.obj,
});
...
V_LeagueSchema.statics.getMostRecentSeasonNumber = async function () {
...
V_LeagueSchema.statics.queryForIfSeasonExists = async function (seasonNumber) {
...
V_LeagueSchema.statics.getConferences = async function (seasonNumber) {
...
V_LeagueSchema.statics.getSingleTeamStandings = async function (
  seasonNumber,
  teamAcronym
) {
  const getSeasonDocument = await this.findOne({ seasonNumber: seasonNumber });
...
V_LeagueSchema.statics.getSortedStandings = async function (seasonNumber) {
  const getSeasonDocument = await this.findOne({ seasonNumber: seasonNumber });
  const standings = getSeasonDocument["standings"];
  standings.sort((a, b) => {
...
V_LeagueSchema.statics.getSortedConferenceStandings = async function(seasonNumber, conferenceName){
  const getSeasonDocument = await this.findOne({ seasonNumber: seasonNumber });
...
V_LeagueSchema.statics.getFieldData = async function (
  seasonNumber,
  paramtersList,
  requestedDataObject
) {
...
const V_Season = models.v_season || model("v_season", V_LeagueSchema);

export default V_Season;