...
const P_LeagueSchema = new Schema({
  ...SeasonSchema.obj,
});
...
P_LeagueSchema.statics.getMostRecentSeasonNumber = async function () {
...
P_LeagueSchema.statics.queryForIfSeasonExists = async function (seasonNumber) {
...
P_LeagueSchema.statics.getConferences = async function (seasonNumber) {
...
P_LeagueSchema.statics.getSingleTeamStandings = async function (
  seasonNumber,
  teamAcronym
) {
  const getSeasonDocument = await this.findOne({ seasonNumber: seasonNumber });
...
P_LeagueSchema.statics.getSortedStandings = async function (seasonNumber) {
  const getSeasonDocument = await this.findOne({ seasonNumber: seasonNumber });
  const standings = getSeasonDocument["standings"];
  standings.sort((a, b) => {
...
P_LeagueSchema.statics.getSortedConferenceStandings = async function(seasonNumber, conferenceName){
  const getSeasonDocument = await this.findOne({ seasonNumber: seasonNumber });
...
P_LeagueSchema.statics.getFieldData = async function (
  seasonNumber,
  paramtersList,
  requestedDataObject
) {
...
const P_Season = models.p_Season || model("p_Season", P_LeagueSchema);

export default P_Season;
