// this method switches home and away team stats
// if players accidentally submit a game state where they played both games in same venue
// example supposed to play one home/away game but played both home/home.
// so this swaps home stats with away stats and vice versa
import readGoalieAttributes from "./read-csv/read-goalie-attributes";
import readSkatersAttributes from "./read-csv/read-skater-attributes";
import readTeamPositionCounts from "./read-csv/read-team-position-counts";
...
async function readBinaryGameStateReversed(
  file,
  seasonNumber,
  gameType,
  leagueName,
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      try {
        ...
        // Home
        const homeGoals = d[50682 + offset];
        const homePPGoals = d[50672 + offset];
        const homePPTries = d[50674 + offset];
        const homePPTime1 = d[51522 + offset];
        const homePPTime2 = d[51523 + offset] * 256;
        const homePPTime = homePPTime1 + homePPTime2;
        const homePPShots = d[51524 + offset];
        const homeSHGoals = d[51526 + offset];
        const homeSHGoalsAgainst = d[50656 + offset];
        const homeBreakGoals = d[51530 + offset];
        const homeBreakTries = d[51528 + offset];
        const homeOneTimerGoals = d[51534 + offset];
        const homeOneTimerTries = d[51532 + offset];
        const homePenShotGoals = d[51538 + offset];
        const homePenShotTries = d[51536 + offset];
        const homeFaceoffWins = d[50684 + offset];
        const homeChecks = d[50686 + offset];
        const homePenalties = d[50676 + offset];
        const homePIM = d[50678 + offset];
        const homeAttackZoneTime1 = d[50680 + offset];
        const homeAttackZoneTime2 = d[50681 + offset] * 256;
        const homeAttackZoneTime = homeAttackZoneTime1 + homeAttackZoneTime2;
        const homePassComps = d[50690 + offset];
        let homePassTries = d[50688 + offset];
        ...
        if (homePassTries < homePassComps) {
          ...
          const homeGoalsP1 = d[51506 + offset];
          const homeGoalsP2 = d[51508 + offset];
          const homeGoalsP3 = d[51510 + offset];
          const homeGoalsOT = d[51512 + offset];
          const homeShotsP1 = d[51514 + offset];
          const homeShotsP2 = d[51516 + offset];
          const homeShotsP3 = d[51518 + offset];
          const homeShotsOT = d[51520 + offset];
          const homeTeam = teamCodesDict[d[48982 + offset]];
        ...
        // Away
        const awayGoals = d[49812 + offset];
        const awayPPGoals = d[49802 + offset];
        const awayPPTries = d[49804 + offset];
        const awayPPTime1 = d[50652 + offset];
        const awayPPTime2 = d[50653 + offset] * 256;
        const awayPPTime = awayPPTime1 + awayPPTime2;
        const awayPPShots = d[50654 + offset];
        const awaySHGoals = d[50656 + offset];
        const awaySHGoalsAgainst = d[51526 + offset];
        const awayBreakGoals = d[50660 + offset];
        const awayBreakTries = d[50658 + offset];
        const awayOneTimerGoals = d[50664 + offset];
        const awayOneTimerTries = d[50662 + offset];
        const awayPenShotGoals = d[50668 + offset];
        const awayPenShotTries = d[50666 + offset];
        const awayFaceoffWins = d[49814 + offset];
        const awayChecks = d[49816 + offset];
        const awayPenalties = d[49806 + offset];
        const awayPIM = d[49808 + offset];
        const awayAttackZoneTime1 = d[49810 + offset];
        const awayAttackZoneTime2 = d[49811 + offset] * 256;
        const awayAttackZoneTime = awayAttackZoneTime1 + awayAttackZoneTime2;
        const awayPassComps = d[49820 + offset];
        let awayPassTries = d[49818 + offset];
        ...
        if (awayPassTries < awayPassComps) {
          ...
          const awayGoalsP1 = d[50636 + offset];
          const awayGoalsP2 = d[50638 + offset];
          const awayGoalsP3 = d[50640 + offset];
          const awayGoalsOT = d[50642 + offset];
          const awayShotsP1 = d[50644 + offset];
          const awayShotsP2 = d[50646 + offset];
          const awayShotsP3 = d[50648 + offset];
          const awayShotsOT = d[50650 + offset];
          const awayTeam = teamCodesDict[d[48980 + offset]];
        ...
        const teamPenDict = {
            ...
            24: "Away Slashing",
            26: "Away Roughing",
            28: "Away Cross-Checking",
            30: "Away Hooking",
            32: "Away Tripping",
            34: "Away Interference",
            36: "Away Holding",
            38: "Away Fighting",
            146: "Home Boarding",
            150: "Home Charging",
            152: "Home Slashing",
            154: "Home Roughing",
            156: "Home Cross-Checking",
            158: "Home Hooking",
            160: "Home Tripping",
            162: "Home Interference",
            164: "Home Holding",
            166: "Home Fighting",
          };
...
export default readBinaryGameStateReversed;
